...
export interface UseSentListOptions {
  pageSize?: number;
  sort?: string;
}

export interface IUseSentList {
...
export function useSentList(options?: UseSentListOptions): IUseSentList {
  ...
  const { action: loadMessages, loading: loadingMessages } = useAsync<IPushMessageSearchCriteria>(async (_query) => {
    searchQuery.value = {
      ...searchQuery.value,
      ...(_query || {}),
      statuses: ["Sent"], // Always filter for sent messages
    };
